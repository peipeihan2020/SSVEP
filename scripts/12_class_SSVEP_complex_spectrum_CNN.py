...
for subject in range(0, 10):
    ...
    filtered_data = pre.get_filtered_eeg(eeg, 6, 80, 4, sample_rate)
    eeg = []

    window_len = 1 
    shift_len = 1
    
    segmented_data = pre.get_segmented_epochs(filtered_data, window_len, shift_len, sample_rate)
    filtered_data = []
    ...
    subject_labels = (matlib.repmat(class_labels, total_epochs_per_class, 1).T).ravel()
    subject_labels = to_categorical(subject_labels)
    if data is None:
        data = train_data
        labels = subject_labels
    else:
        data = np.vstack([data, train_data])
        labels = np.vstack([labels, subject_labels])


train_data, train_labels, test_data, test_labels = train_test_split(data, labels, test_size=0.2, random_state=42)
